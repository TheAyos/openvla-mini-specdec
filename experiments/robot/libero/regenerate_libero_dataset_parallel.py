...
def _process_task(task_id, task_suite_name, raw_dir, target_dir):
    """Replay one task's demos and write a regenerated HDF5."""
    ...
    orig_data_path = os.path.join(raw_dir, f"{task.name}_demo.hdf5")
    assert os.path.exists(orig_data_path), f"Cannot find raw data file {orig_data_path}."
    orig_data_file = h5py.File(orig_data_path, "r")
    orig_data = orig_data_file["data"]

    # Create new HDF5 file for regenerated demos
    new_data_path = os.path.join(target_dir, f"{task.name}_demo.hdf5")
...
def _process_task_wrapper(args_tuple):
    task_id, task_suite_name, raw_dir, target_dir = args_tuple
    return _process_task(task_id, task_suite_name, raw_dir, target_dir)


def main(args):
    print(f"Regenerating {args.libero_task_suite} dataset with {args.num_workers} workers!")
    ...
    # Get task suite to enumerate tasks
    benchmark_dict = benchmark.get_benchmark_dict()
    task_suite = benchmark_dict[args.libero_task_suite]()
    num_tasks_in_suite = task_suite.n_tasks

    # Parallelize per task (each process handles one task's demos and writes its own HDF5)
    ctx = mp.get_context("spawn")
    job_args = [
        (tid, args.libero_task_suite, args.libero_raw_data_dir, args.libero_target_dir)
        for tid in range(num_tasks_in_suite)
    ]
    with ctx.Pool(processes=args.num_workers) as pool:
        results = []
        for res in tqdm.tqdm(pool.imap_unordered(_process_task_wrapper, job_args), total=num_tasks_in_suite):
            # Per-task completion log
            print(
                f"[task done] {res['task_description']} "
                f"(replays={res['num_replays']}, success={res['num_success']}, noops={res['num_noops']})",
                flush=True,
            )
            results.append(res)

    # Aggregate results
    num_replays = sum(r["num_replays"] for r in results)
    num_success = sum(r["num_success"] for r in results)
    num_noops = sum(r["num_noops"] for r in results)
    for r in results:
        metainfo_json_dict.update(r["metainfo"])
        print(f"Saved regenerated demos for task '{r['task_description']}'")

    # Write final metainfo
    with open(metainfo_json_out_path, "w") as f:
        json.dump(metainfo_json_dict, f, indent=2)

    print(f"Dataset regeneration complete! Saved new dataset at: {args.libero_target_dir}")
    print(f"Total # episodes replayed: {num_replays}, Total # successes: {num_success} ({num_success / max(num_replays,1) * 100:.1f} %)")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Number of parallel workers (processes) to use; each handles one task.",
    )
