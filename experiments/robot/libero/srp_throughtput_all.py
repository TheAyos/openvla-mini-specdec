...
Usage:
    ...
    python -m openvla-mini.experiments.robot.libero.srp_throughtput_all
...
class ThroughputConfig:
    ...
    model_type: str = "prismatic"                                    # Model type: "openvla", "prismatic", or "openvla_oft"
    
    # OpenVLA-OFT specific
    openvla_oft_root: str = "../openvla-oft"                         # Path to openvla-oft root
    ...
    num_diffusion_steps_train: int = 100                             # Number of diffusion steps used for training
    num_diffusion_steps_inference: int = 10                          # Number of diffusion steps used for inference
    ...
    model_family: str = "prismatic"                                  # Model family for environment compatibility
...
def measure_throughput_with_single_libero_task(cfg: ThroughputConfig) -> dict:
    ...
    assert cfg.model_type in ["openvla", "prismatic", "openvla_oft"], f"Invalid model_type: {cfg.model_type}"

    if cfg.model_type == "openvla_oft":
        # Import openvla-oft utils dynamically
        oft_root = Path(cfg.openvla_oft_root).resolve()
        if not oft_root.exists():
             raise FileNotFoundError(f"OpenVLA-OFT root not found at {oft_root}")
        
        # Add to sys.path to allow imports from openvla-oft
    ...
    if cfg.model_type == "prismatic":
        ...
        # Import openvla-oft utils
        from experiments.robot.openvla_utils import get_action_head, get_proprio_projector, get_noisy_action_projector, get_vla_action as get_oft_vla_action
    ...
    if cfg.model_type == "openvla":
        ...
        def run_inference():
            return get_vla_action(
                ...
                # profile_dir="experiments/logs/prof_trace_openvla_compile_max_autotune_fullgraph"
            )
        ...
        def run_inference():
            return get_prismatic_vla_action(
                ...
                cfg.unnorm_key,
                center_crop=cfg.center_crop,
                # profile_dir="experiments/logs/prof_trace_prismatic_compile_max_autotune_fullgraph"
            )
        ...
        if cfg.use_proprio:
             # Assuming proprio_dim is 8 as seen in run_libero_eval.py
    ...
    # took from http://docs.pytorch.org/tutorials/intermediate/torch_compile_tutorial.html
    def timed(fn):
    ...
    total_time = 0
    for i in range(cfg.num_iterations):
        res, dt = timed(lambda: run_inference())
        total_time+=dt
        if (i + 1) % 10 == 0:
            print(f"  Progress: {i + 1}/{cfg.num_iterations} -> {res} @ {dt:.02f}s")
