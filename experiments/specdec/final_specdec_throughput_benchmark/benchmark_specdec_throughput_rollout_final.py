...
def openvla_to_prismatic_image(openvla_image: np.ndarray) -> np.ndarray:
    """
    Convert OpenVLA-preprocessed image to Prismatic-preprocessed image.
    
    OpenVLA preprocessing: 180° rotation (flip both axes) = img[::-1, ::-1]
    Prismatic preprocessing: vertical flip only = np.flipud(img)
    
    To convert: flip horizontal axis to undo the extra flip
    openvla_image[:, ::-1] = prismatic_image
    """
    return openvla_image[:, ::-1].copy()
...
class VLASpeculativeDecoderBatchedLM:
    ...
    def _prepare_draft_inputs(self, image: Image.Image, instruction: str) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # Convert OpenVLA-preprocessed image to Prismatic format for draft model
        # OpenVLA: 180° rotation, Prismatic: vertical flip only
        # Difference: horizontal flip, so we undo it here
        image_np = np.array(image)
        prismatic_image_np = openvla_to_prismatic_image(image_np)
        prismatic_image = Image.fromarray(prismatic_image_np).convert("RGB")
        
        image_transform = self.draft.vision_backbone.get_image_transform()
        pixel_values = image_transform(prismatic_image)
...
def run_rollout_benchmark(method: str, decoder=None) -> Dict[str, Any]:
    ...
    while t < max_steps + cfg.num_steps_wait:
        ...
        # Get observation - use correct preprocessing per model type
        # OpenVLA needs 180° rotation, Prismatic needs vertical flip only
        if method == "target":
            img = get_libero_image(obs, 224, model_family="openvla")
        elif method == "draft":
            img = get_libero_image(obs, 224, model_family="prismatic")
        elif method == "specdec":
            # For specdec, target model needs openvla preprocessing
            # The draft model's transform handles its own needs from the PIL image
            img = get_libero_image(obs, 224, model_family="openvla")
...
plt.title(r'$\\alpha$, speedup vs $r$ (relaxation threshold)' + f' ($\\gamma$={cfg.gamma}, rollout)', fontsize=12)
fig.tight_layout()
...
for gamma in gamma_values:
    ...
    specdec_decoder_gamma = VLASpeculativeDecoderBatchedLM(
        ...
        gamma=gamma,
        temperature=cfg.temperature,
        relaxed_acceptance_r=7,  # Fixed: was incorrectly using cfg.relaxed_acceptance_r
    )
...
for gamma in gamma_values:
    ...
    specdec_decoder_gamma = VLASpeculativeDecoderBatchedLM(
        ...
        gamma=gamma,
        temperature=cfg.temperature,
        relaxed_acceptance_r=255,  # Fixed: was incorrectly using cfg.relaxed_acceptance_r
    )
